...
const methodSpeciation = await readFile(
  join(__dirname, '../src/logic/CharacteristicName-MethodSpeciation.json')
)
...
const sampleFraction = await readFile(
  join(__dirname, '../src/logic/CharacteristicName-ResultSampleFraction.json')
)
  .then((res) => JSON.parse(res).if.properties.CharacteristicName.enum)
  .catch(() => [])
...
const unitDescriptionsOverrides = await readFile(
  join(__dirname, '../src/lookup/MeasurementUnit.json')
)
  .then((res) => JSON.parse(res))
  .catch(() => ({}))
