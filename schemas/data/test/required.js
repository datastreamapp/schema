...
test('Should require properties', async (t) => {
  ...
  assert.equal(
    checkProperty(validate.errors, 'required', 'ActivityStartDate'),
    true
  )
  assert.equal(
  ...
  assert.equal(
    checkProperty(validate.errors, 'required', 'ResultValueType'),
    true
  )
})
...
test('Should reject ResultValue AND NOT ResultUnit', async (t) => {
  ...
  assert.equal(valid, false)
  assert.equal(
    checkProperty(validate.errors, 'dependencies', 'ResultUnit'),
    true
  )
})
...
test('Should reject WellID AND NOT WellIDContext', async (t) => {
  ...
  assert.equal(valid, false)
  assert.equal(
    checkProperty(validate.errors, 'dependencies', 'WellIDContext'),
    true
  )
})
...
test('Should reject WellDepthMeasure AND NOT WellDepthUnit', async (t) => {
  ...
  assert.equal(valid, false)
  assert.equal(
    checkProperty(validate.errors, 'dependencies', 'WellDepthUnit'),
    true
  )
})
